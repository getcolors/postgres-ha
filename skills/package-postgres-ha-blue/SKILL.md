---
name: package-postgres-ha-blue
description: Provision and operate a three-node PostgreSQL failover cluster on DigitalOcean with Patroni, etcd, HAProxy, Cloudflare DNS, and pgBackRest point-in-time recovery to Cloudflare R2, using Blue.
license: MIT
---
...
```sh
./blue build              # render the work directory only — contact nothing
./blue create --dry-run   # walk the graph, skip every side effect
./blue create             # converge
...
```sh
./blue status             # patronictl list — members, roles, replication lag
./blue switchover         # planned handover to a healthy standby
./blue failover --node 2  # unplanned promotion, dispatched via a live node
./blue backup             # run the full backup now, on the leader
./blue verify-restore     # run the verified restore now, on a standby
./blue psql               # a session on the current primary, via HAProxy
```
