---
name: package-postgres-ha-red
description: Provision and operate a three-node PostgreSQL failover cluster on DigitalOcean with Patroni, etcd, HAProxy, Cloudflare DNS, and pgBackRest point-in-time recovery to Cloudflare R2, using Red.
license: MIT
---
...
```sh
./red build              # render the work directory only — contact nothing
./red create --dry-run   # walk the graph, skip every side effect
./red create             # converge
...
```sh
./red status             # patronictl list — members, roles, replication lag
./red switchover         # planned handover to a healthy standby
./red failover --node 2  # unplanned promotion, dispatched via a live node
./red backup             # run the full backup now, on the leader
./red verify-restore     # run the verified restore now, on a standby
./red psql               # a session on the current primary, via HAProxy
```
